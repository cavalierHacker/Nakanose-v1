...
//Nakanose v2.
//
//Created by Carlos E. Vanegas on 09/01/2013
//Copyright Black Squirrel TM 2013 All Rights Reserved.
//Updated Nov 9th 2013
//
...
Hammerhead::Hammerhead() {
	...
	_floatingTimerMax = 2;
	_floatingTimer = 0;
    _speed = HAMMERHEAD_INITIAL_SPEED;
    _maxSpeed = HAMMERHEAD_INITIAL_SPEED;
    _floating = false;
    _nextPosition = CCPointZero;
	_nextPosition.y = _screenSize.height * 0.6f;
	_state = kHammerheadMoving;
    _jumping = false;
    _hasFloated = false;
...
Hammerhead * Hammerhead::create() {
	
	Hammerhead * hammerhead = new Hammerhead();
	if (hammerhead && hammerhead -> initWithSpriteFrameName("hammerhead_1.png")) {
		
		hammerhead->autorelease();
		hammerhead->setSize();
        hammerhead->initHammerhead();
		
        return hammerhead;
		
	}
	
	CC_SAFE_DELETE(hammerhead);
	return NULL;

}


void Hammerhead::update (float dt) {
	...
	switch (_state) {
		case kHammerheadMoving:
			_vector.y -= GRAVITY;
            if (_hasFloated) _hasFloated = false;
            break;
            
        case kHammerheadFalling:
			//&& _vector.y < 0
            if (_floating ) {
    ...
    if (_jumping) {
        _state = kHammerheadFalling;
        _vector.y += HAMMERHEAD_JUMP * 0.25f;
        if (_vector.y > HAMMERHEAD_JUMP ) _jumping = false;
    }
    ...
    void Hammerhead::setFloating (bool value) {
        ...
        if (value) {
            ...
            _vector.y += HAMMERHEAD_JUMP * 0.5f;
        } else {
    ...
    void Player::initHammerhead() {
        ...
        for(i = 1; i <= 3; i++) {
            char szName[100] = {0};
            sprintf(szName, "hammerhead_%i.png", i);
        ...
        CCFiniteTimeAction* easeSwing = CCSequence::create(CCEaseInOut::create(CCRotateTo::create(0.8f, -10), 2),
